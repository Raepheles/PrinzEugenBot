...
if (config.traceFileLogsOn)
  logConfig.categories.default.appenders.push("trace");
...
async function start() {
  ...
  try {
    ...
    logger.error(
      `Got error while parsing the data and there is no data present. Exiting the app. Error: ${err}`
    );
    process.exit(1);
  }
...
async function loadData() {
  ...
  if (!obj.date || !obj.ships || !obj.unreleasedShips || !obj.equipments)
    throw Error(`Invalid ${config.dataFileName}.json`);
