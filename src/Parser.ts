...
export class Parser {
  ...
  public async parseShips(): Promise<ShipData> {
    ...
    this.logger.info(
      `Parsed ship list in ${getTimeFromMs(Date.now() - start, true)}`
    );
    const ships: Ship[] = [];
    ...
    for (const shipWrapper of shipWrappers) {
      try {
        ...
        this.logger.trace(
          `Parsed ${shipWrapper.name} in ${getTimeFromMs(
            Date.now() - shipParseStart,
            true
          )}`
        );
        ...
        const errStr = `${err}`;
        this.logger.error(
          `Error while parsing ${shipWrapper.name}: ${
            errStr.length >= 128 ? errStr.substr(0, 127) : errStr
          }`
        );
      }
    }
    this.logger.info(
      `Parsed ${total - failed} ships out of ${total} in ${getTimeFromMs(
        Date.now() - start,
        true
      )}`
    );
  ...
  public async parseEquipments(): Promise<Equipment[]> {
    ...
    this.logger.info(
      `Parsed equipment list in ${getTimeFromMs(Date.now() - start, true)}`
    );
    ...
    for (const equipmentName of equipmentNames) {
      try {
        ...
        this.logger.trace(
          `Parsed ${equipmentName} in ${getTimeFromMs(
            Date.now() - equipmentParseStart,
            true
          )}`
        );
        ...
        const errStr = `${err}`;
        this.logger.error(
          `Error while parsing ${equipmentName}: ${
            errStr.length >= 128 ? errStr.substr(0, 127) : errStr
          }`
        );
      }
    ...
    this.logger.info(
      `Parsed ${total - failed} equipments out of ${total} in ${getTimeFromMs(
        Date.now() - start,
        true
      )}`
    );
  ...
  private async getEquipmentList(): Promise<string[]> {
    ...
    const list =
      mainDoc.getElementById("Equipment_Lists")?.parentElement
        ?.nextElementSibling?.children;
    if (!list)
      throw Error("Couldn't find <ul> children in equipment lists page.");
    ...
    for (const li of list) {
      ...
      const tbody = doc
        .querySelector(".tabbertab > table")
        ?.getElementsByTagName("tbody")
        .item(0);
      if (!tbody)
        throw Error(
          "Couldn't find tbody under first element with tabbertab class."
        );
  ...
  private async getEquipment(equipmentName: string): Promise<Equipment> {
    ...
    if (!eqBoxes)
      throw Error(
        `Couldn't find elements with class 'eq-box' for ${equipmentName}`
      );
    ...
    if (!eqHead)
      throw Error(`Couldn't find eq-head under eq-box for ${equipmentName}`);

    const eqInfo = eqHead.querySelector(".eq-gen > .eq-info");
    if (!eqInfo)
      throw Error(`Couldn't find eq-info under eq-gen for ${equipmentName}}`);

    const eqInfoTbody = eqInfo.getElementsByTagName("tbody").item(0);
    if (!eqInfoTbody)
      throw Error(`Couldn't find tbody under eq-info for ${equipmentName}`);

    const tds = eqInfoTbody.getElementsByTagName("td");
    if (!tds)
      throw Error(
        `Couldn't find any td under tbody under eq-info for ${equipmentName}`
      );
    ...
    const { mainUsers, secondaryUsers } = this.getEquipmentUsers(
      firstEqBox,
      equipmentName
    );
    ...
    for (const eqBox of eqBoxes) {
      ...
      if (!tier)
        throw Error(`Couldn't find equipment tier for ${equipmentName}`);

      const rarity = eqBox
        .getElementsByClassName("eq-head")
        .item(0)
        ...
        .replace(/★/g, "");
      if (!rarity)
        throw Error(`Couldn't get rarity for ${equipmentName} - ${tier}`);
  ...
  private getEquipmentStats(
    ...
    const eqStatsTbody = eqBox
      .getElementsByClassName("eq-stats")
      .item(0)
      ?.getElementsByTagName("tbody")
      .item(0);
    if (!eqStatsTbody)
      throw Error(`Couldn't find eq-stats tbody for ${eqName} - ${eqTier}`);
    ...
    for (const child of eqStatsTbody.children) {
      ...
      const key =
        child.firstElementChild?.firstElementChild?.getAttribute("alt") ||
  ...
  private getEquipmentUsers(eqBox: Element, eqName: string): EquipmentUsers {
    const trs = eqBox
      .getElementsByClassName("eq-fits")
      .item(0)
      ?.getElementsByTagName("tr");
    if (!trs)
      throw Error(`Couldn't find <tr> elements under eq-fits for ${eqName}`);
  ...
  private async getShipList(): Promise<ShipWrapper[]> {
    ...
    for (let i = 0; i < elements.length; i += 2) {
      if (elements.length <= i + 1)
        throw Error("Ship elements out of boundary. Wiki changed?");
      ...
      if (!e.firstElementChild)
        throw Error(
          "Element doesn't have first child (name element). Wiki changed?"
        );
  ...
  private async getShip(
    shipWrapper: ShipWrapper
  ): Promise<Ship | UnreleasedShip> {
    const body = await request(
      `${this.wikiUrl}/${this.urlEncode(shipWrapper.name)}`
    );
    const doc = new JSDOM(body).window.document;
    ...
    if (!els) {
      ...
      for (const el of els) {
        ...
        if (!hel) continue;
        if (
          hel.tagName.toLowerCase() === "div" &&
          hel.style.display.toLowerCase() === "none"
        )
          unreleased = true;
      }
    }
    ...
    const type = tables
      .item(1)
      ?.firstElementChild?.children.item(2)
      ?.children.item(1)
      ?.textContent?.trim();
    ...
    if (!nationality)
      throw Error(`Error while getting nationality of ${shipWrapper.name}`);
    ...
    if (!shipClass)
      throw Error(`Error while getting class of ${shipWrapper.name}`);
  ...
  private getShipSkills(doc: Document, shipName: string): ShipSkill[] {
    ...
    for (const table of tables) {
      if (table.textContent?.includes("Skills")) {
        ...
        if (!tbody)
          throw Error(`Couldn't find tbody under skills table for ${shipName}`);
        ...
        for (let i = 1; i < skillElements.length; i++) {
          ...
          if (!skillStyle)
            throw Error(
              `Skill type background color not found for ${shipName}.`
            );
  ...
  private getShipStats(
    ...
    // const el = selector.length >= 0 ? selector.item(0) : undefined;
    if (!el) throw Error(`Couldn't find ${key} stats for ${shipName}`);

    const tables = el.getElementsByTagName("table");
    if (tables.length === 0)
      throw Error(`Tables length is 0. Stats: ${key}, Ship: ${shipName}`);
  ...
  private async getShipImages(
    mainDoc: Document,
    shipName: string
  ): Promise<ShipImages> {
    const body = await request(
      `${this.wikiUrl}/${this.urlEncode(shipName)}/Gallery`
    );
    const doc = new JSDOM(body).window.document;

    const tabbers = doc.getElementsByClassName("tabber");
    if (!tabbers || tabbers.length === 0)
      throw Error(
        `Couldn't find element with class name 'tabber' for Gallery of ${shipName}`
      );
    const imageElements = tabbers[0].children;

    if (!imageElements || imageElements.length === 0)
      throw Error(
        `Couldn't find any element with 'tabbertab' class. ${shipName}`
      );
    ...
    for (const el of imageElements) {
      ...
      if (!name)
        throw Error(
          `Couldn't find 'title' attribute on one of the skill tabs for ${shipName}`
        );
      const tabInTab = el.getElementsByClassName("tabbertab");
      if (tabInTab && tabInTab.length > 0) {
        for (const el2 of tabInTab)
          this.insertImages(el2, shipName, images, el, name);
      } else {
  ...
  private getMisc(doc: Document, shipName: string): ShipMisc {
    ...
    if (!tbody)
      throw Error(`Couldn't get tbody from misc table for ${shipName}`);
  ...
  private getConstruction(doc: Document, shipName: string): ShipConstruction {
    ...
    if (!el)
      throw Error(
        `Couldn't get element with id 'Construction' for ${shipName}`
      );
    ...
    const constTime = tbody?.children
      .item(1)
      ?.children.item(0)
      ?.textContent?.trim();
    if (!tbody)
      throw Error(`Couldn't get tbody from Construction table for ${shipName}`);
    if (!constTime)
      throw Error(
        `Couldn't get construction time from construction table for ${shipName}`
      );

    let info =
      tbody.children.length === 6
        ? tbody.lastElementChild?.children[1].textContent
        : "";
    ...
    const dropText =
      dropLocations.length !== 0 ? dropLocations.join("\n") : undefined;
    return {
      text: construction,
      drop: dropText,
      additional: info,
